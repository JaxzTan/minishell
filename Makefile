...
# Readline path detection
ifeq ($(shell test -d /opt/homebrew/opt/readline && echo exists),exists)
...
IFLAGS		=	-I$(INC) -I$(READLINE_PREFIX)/include
LFLAGS		=	-L$(READLINE_PREFIX)/lib
DSYM		=	-g3
FSAN		=	-fsanitize=address $(DSYM)
CFLAGS		=	$(WFLAGS) $(IFLAGS) -D_DARWIN_C_SOURCE
CC			=	cc
RM			=	rm -rf
READLINE	=	$(LFLAGS) -lreadline
